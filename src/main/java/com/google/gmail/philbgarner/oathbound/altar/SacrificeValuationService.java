 ...
 *
 * Per enchantment, {@code baseWeight = (weightScale / maxLevel) * rarityMultiplier}, so maxing out any
 * single enchantment is worth exactly {@code weightScale * rarityMultiplier} regardless of how many
 * levels it has (normalizes a five-level enchantment like Sharpness against a one-level enchantment
 * like Mending without a hardcoded per-enchantment level table) - but the ceiling itself now also
 * scales with how significant/rare the enchantment actually is, via {@code rarityMultipliers}, rather
 * than every maxed-out enchantment being worth identically regardless of how easy it was to max. An
 * item's value is the sum of {@code baseWeight * level} across its enchantments.
 *
 ...
 * The rarity multiplier applies once per enchantment type (it's part of the ceiling), not per
 * occurrence - repeat-decay still does the work of discouraging volume-stuffing on top of it.
 */
public final class SacrificeValuationService {
    ...
    public static long valueOf(List<Map<String, Integer>> depositedItemEnchants, EnchantmentMaxLevelLookup maxLevels,
                                EnchantmentRarityLookup rarityMultipliers, double weightScale, double repeatDecay) {
        ...
        for (Map.Entry<String, List<Integer>> entry : levelsByEnchant.entrySet()) {
            ...
            double baseWeight = (weightScale / maxLevel) * rarityMultipliers.multiplierOf(entry.getKey());
